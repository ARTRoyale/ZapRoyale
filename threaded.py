...
class ThreadedServer(object):
    def __init__(self, host, port):
        ...
        print('Servak k portu ', self.port)
    def listen(self):
        self.sock.listen(5)
        while True:
            print('Slushau podklucheniya!')
            client, address = self.sock.accept()
            print('novoe podkluchenie ot ', address, '!')
            client.settimeout(600) #10 minute timeout
            threading.Thread(target = self.listenToClient,args = (client,address)).start()

            # начинаем слушать клиент

    def listenToClient(self, client, address):
        size = 2048
        while True:
            try:
                print('\r\nZhdu infu ot ', address)
                ...
                if message_id == 10101:
                    resd = data
                print('Poluchena infa ot ', address, ':\r\n', 'message-id: ', message_id,'\r\n')
                if content: # получаем инфу от клиента
                    print(content)
                if data:
                    print('\r\nGenerated Response:\r\n', resd)
                    ...
                    raise error('Klient otkluchilsa')
            except:
                client.close()
                return False
...
if __name__ == "__main__":
    port_num = 9339
    print('Zapusk servaka na porte ', port_num)
